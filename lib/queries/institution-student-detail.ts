...
export async function getInstitutionStudentDetail(internshipId: string) {
  ...
  const { data: profile, error: profileError } = await supabase
    .from("profiles")
    .select("id, role, is_active, institution_id")
    .eq("id", user.id)
    .single();
  ...
  if (!profile.is_active || profile.role !== "instituicao" || !profile.institution_id) {
    return {
      detail: null,
  ...
  if (internship.institution_id !== profile.institution_id) {
    return {
      detail: null,
      error: "Este estágio não pertence à sua instituição.",
    };
  }
